...
std::string PrepareExpressionFromXML(std::string filename)
{
  ...
  for( TiXmlElement* n_Parameter = n_AppNode->FirstChildElement("parameter"); n_Parameter != NULL;
    ...
    if(n_Values)
      ...
      if (type == "OutputImage")
        {
        std::string stype = GetChildNodeTextOf(n_Parameter, "pixtype");
        expression.append(" ");
        expression.append(stype);
        }
      }
    }
  ...
  for( TiXmlElement* n_Parameter = n_AppNode->FirstChildElement("parameter"); n_Parameter != NULL;
    ...
    if(n_Values)
      ...
      if (type == "OutputImage")
        {
        std::string stype = GetChildNodeTextOf(n_Parameter, "pixtype");
        expression.push_back(CleanWord(stype));
        }
      }
    }
...
int main(int argc, char* argv[])
{
  ...
  typedef otb::Wrapper::MPICommandLineLauncher LauncherType;
  ...
  if (launcher->Load(vexp) == true)
    ...
    if (myrank == 0)
      std::cout << "Total number of MPI process: " << nprocs << std::endl;

    launcher->SetMyRank(myrank);
    launcher->SetNProcs(nprocs);
    int process_status = launcher->ExecuteAndWriteOutput();

    // Gather then broadcast the launcher status
    MPI_Reduce(&process_status, &general_status, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Bcast(&general_status, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // Terminate MPI session
    MPI_Finalize();
    general_status -= nprocs;
